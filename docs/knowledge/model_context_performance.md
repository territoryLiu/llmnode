...
### `models/Qwen/Qwen3.6-35B-A3B-AWQ-4bit`
...
- 最大已验证上下文: `262144`
- 是否单条通过 256K: 从当前在线实例容量看是
- 备注:
  - 本轮先记录当前在线实例的启动容量与显存分配观测
  - 结论来自 `2026-05-19` 当前运行容器启动日志，不是手工估算

#### KV cache 与并发估算

- Model loading took: `22.4 GiB memory`
- Available KV cache memory: `15.03 GiB`
- GPU KV cache size: `196,416 tokens`
- `256K tokens/request` 理论最大并发: `2.96x`
- 单条 `256K` 请求的 KV cache 成本: 约 `5.08 GiB`
- 工程安全并发建议:
  - `256K` 按 `2` 个请求规划
  - 不建议按 `3` 个 `256K` 请求规划常态并发
- 备注:
  - `15.03 / 2.96 ≈ 5.08 GiB`，这是当前配置下单条 `256K` 的近似 KV 成本
  - 模型权重 `22.4 GiB` 加单条 `256K` KV `5.08 GiB` 后约 `27.48 GiB`
  - 这说明“单条 `256K` 只给 `30 GiB` 显存预算”虽然从静态加总上接近可行，但余量偏小，不适合作为稳妥工程预算
  - 更稳妥的理解是：
    - `30 GiB` 不是完全不可能
    - 但它更接近边界值，不应被视为保守安全值
    - 如果目标是稳定单条 `256K`，应预留额外 runtime / allocator / graph capture / 碎片余量

#### 单并发阶梯测试

| 阶梯 | 实测 prompt tokens | completion tokens | 总时延 ms | 估算 completion tok/s | 结论 |
|------|--------------------|-------------------|-----------|------------------------|------|
| 4K   | 未测 | 未测 | 未测 | 未测 | 当前仅记录容量与启动观测 |
| 32K  | 未测 | 未测 | 未测 | 未测 | 当前仅记录容量与启动观测 |
| 64K  | 未测 | 未测 | 未测 | 未测 | 当前仅记录容量与启动观测 |
| 128K | 未测 | 未测 | 未测 | 未测 | 当前仅记录容量与启动观测 |
| 256K | 未测 | 未测 | 未测 | 未测 | 当前仅记录容量与启动观测 |

#### 观察

- 当前配置下，模型本体显存明显高于 `27B-FP8`，但 KV cache 预算也明显更大。
- 从容量角度看，这个 `AWQ-4bit` 配置已经具备稳定单条 `256K` 的基础条件。
- 但 `256K` 的理论并发虽然接近 `3x`，工程上仍应按 `2x` 规划，避免输出长度、工具调用、碎片和 warmup 抖动把实例推到边界外。
- 如果只按“模型权重 + 单条 `256K` KV”静态相加，单条 `256K` 约需 `27.48 GiB`。
- 因此把预算说成“`30 GiB` 就够”并不严谨；更准确的说法是：
  - `30 GiB` 接近单条 `256K` 的可行边界
  - 但不属于有明显安全余量的预算
  - 若目标是稳定运行而不是勉强跑通，仍应预留额外显存余量
